/* chafitsoml: fit a SOML (OML with flow) charging curve to pot output */
...
/* The auxiliary function s1(v) defined & used in my PRE paper. */
double s1(double v)
{
	if (v == 0.0) {
		return 1.0;
	} else {
		return (SQRT_PI * (1.0 + (2.0 * v * v)) * erf(v) / (4.0 * v))
		       + exp(- v * v) / 2.0;
	}
}

/* The auxiliary function s2(v) defined & used in my PRE paper. */
double s2(double v)
{
	if (v == 0.0) {
		return 1.0;
	} else {
		return SQRT_PI * erf(v) / (2.0 * v);
	}
}

double log_like(const Data* d, double a, double n, double mime, double v, int Z)
{
	const double chi_3 = 4 * PI * a * a * n * sqrt(KB / (2 * PI * ME));
	const double chi_4 = 4 * PI * EPSILON0 * KB * a / (QE * QE);
	unsigned int i;
	double ll;
	...
	sv[0] = s1(v / sqrt(2.0 * KB * d->T_i[0] / (mime * ME)));
	sv[1] = s2(v / sqrt(2.0 * KB * d->T_i[0] / (mime * ME)));
	rate[0] = chi_3 * sqrt(d->T_e[0]) * (d->t[1] - d->t[0]);
	rate[1] = chi_3 * sv[0] * sqrt(d->T_i[0] / mime) * (d->t[1] - d->t[0]);
	...
	for (i = 1; i < d->N; i++) {
		...
		rate[0] *= time_step;
		sv[0] = s1(v / sqrt(2.0 * KB * d->T_i[i-1] / (mime * ME)));
		sv[1] = s2(v / sqrt(2.0 * KB * d->T_i[i-1] / (mime * ME)));
		rate[1] = chi_3 * sqrt(d->T_i[i-1] / mime);
		rate[1] *= sv[0] - (sv[1] * Z * d->q[i-1] / (chi_4 * d->T_i[i-1]));
		rate[1] *= time_step;
...
void adjust_params_using_gradients(const double* grad, double* param, double* rss, char* points)
{
	unsigned int i;
	#ifdef DEBUG
	const char* param_name[4] = { "a", "n_e", "m_i / m_e", "v" };
	#endif
...
unsigned int fit_by_grad_desc(const char* prog_name, const Data* d, double* param, double* initial_nll, double* final_nll)
{
	...
	if (param[3] < 0.0) {
		fprintf(stderr, "%s initial v %g < 0\n", prog_name, param[3]);
		return 0;
	}

	nll[0] = -log_like(d, param[0], param[1], param[2], param[3], 1);
	*initial_nll = nll[0];

	if (!isfinite(nll[0])) {
		fprintf(stderr, "%s: initial parameter values give invalid NLL\n",
		        prog_name);
		fprintf(stderr, "%s: a = %g, n_e = %g, m_i / m_e = %g, v = %g\n",
		        prog_name, param[0], param[1], param[2], param[3]);
		return 0;
	}
	...
	printf("delta v:       %g\n", dee[3]);
	puts("a           n_e         m_i/m_e v       old NLL "
	     "delta NLL  d(NLL)/da  d(NLL)/dn    d(NLL)/d(MIME) d(NLL)/dv "
	     "RSS a    RSS n_e  RSS MIME");
	printf("%#.6g %#.6g %#.6g %#.6g %#.6g\n",
	       param[0], param[1], param[2], param[3], nll[0]);
	if ((debug_out = fopen("debug-soml.dat", "wb")) == NULL) {
		fprintf(stderr,
		        "%s: can't open debug-soml.dat to write debugging output\n",
		        prog_name);
	} else {
		fprintf(debug_out, "%g %g %g %g %g\n",
		        param[0], param[1], param[2], param[3], nll[0]);
	}
	#endif
	...
	do {
		...
		nll[1] = -log_like(d, param[0] + dee[0], param[1], param[2], param[3], 1);
		nll[2] = -log_like(d, param[0], param[1] + dee[1], param[2], param[3], 1);
		nll[3] = -log_like(d, param[0], param[1], param[2] + dee[2], param[3], 1);
		nll[4] = -log_like(d, param[0], param[1], param[2], param[3] + dee[3], 1);
		grad[0] = (nll[1] - nll[0]) / dee[0];
		grad[1] = (nll[2] - nll[0]) / dee[1];
		grad[2] = (nll[3] - nll[0]) / dee[2];
		grad[3] = (nll[4] - nll[0]) / dee[3];
		...
		nll[5] = -log_like(d, param[0], param[1], param[2], param[3], 1);
		delta_nll = nll[5] - nll[0];

		#ifdef DEBUG
		printf("%#.6g %#.6g %#.6g %#.6g %#.6g %# 9.4f"
		       "  %# .4g %# .6g %# .8f    % #.6f %#.3g %#.3g %#.3g\n",
		       param[0], param[1], param[2], param[3], nll[5], delta_nll,
		       grad[0], grad[1], grad[2], grad[3], rss[0], rss[1], rss[2]);
		if (debug_out != NULL) {
			fprintf(debug_out, "%g %g %g %g, %g\n",
			        param[0], param[1], param[2], param[3], nll[5]);
		}
		#endif
...
void ses_from_log_like(const Data* d, double* param, double final_nll, double* ses)
{
	...
	for (i = 0; i < 4; i++) {
		/* Approximate the second derivative of the negative log
		   likelihood function using a central difference. */
		param[i] -= dee[i];
		nll[0] = -log_like(d, param[0], param[1], param[2], param[3], 1);
		param[i] += (2.0 * dee[i]);
		nll[1] = -log_like(d, param[0], param[1], param[2], param[3], 1);
		cent_diff = nll[1] - (2.0 * final_nll) + nll[0];
		ses[i] = 1.0 / (sqrt(cent_diff) / dee[i]);
		param[i] -= dee[i];
		if (cent_diff < 0.0) {
			fputs("Partial 2nd derivative of NLL function negative, "
			      "suggesting non-convergence\n", stderr);
			ses[i] = 1.0 / 0.0;
		}
	}
}
...
int main(int argc, char* argv[])
{
	...
	double new_param[4];
	double param[4];  /* a, n_e, m_i / m_e, and v respectively */
	double prev_nll;
	double prev_param[4];
	double ses[4];
	...
	if (argc < 6) {
		fprintf(stderr,
		        "Usage: %s [A] [N_E] [V] [START TIME] [FILES]\n", argv[0]);
		return EXIT_FAILURE;
	}
	...
	for (i = 5; i < argc; i++) {
		read_macroscopic_data_file(&d, argv[0], argv[i], atof(argv[4]));
		if (!d.N) {
			return EXIT_FAILURE;
		}
	}
	...
	while (1) {
		...
		new_param[3] = param[3] + (0.8 * (param[3] - prev_param[3]));
		if (new_param[3] < 0.0) {
			new_param[3] = param[3];
		}
		prev_nll = final_nll;
	...
	puts("  a           n_e         m_i / m_e   "
	     "v           NLL      iterations");
	printf("  %11g %11g %11g %#11g %6g %u\n",
	       param[0], param[1], param[2], param[3], final_nll, iterations);
	printf("\xc2\xb1 %11.3g %11.3g %11.3g %11.3g\n",
